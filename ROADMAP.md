...
Replace all native system dialogs with custom-styled components that match the app's design
system (colours, radius, typography). Native Alert.alert() calls and OS-default modals break
visual consistency and look out of place on both iOS and Android. Covers: confirmation dialogs,
destructive action prompts, error messages, and any sheet-style modals using the OS default.


--- Navigation & UX ---

Navigation audit — back gestures, close buttons, and header breadcrumbs
Full audit required before launch covering three areas:
1. Back gestures: all stack screens must support swipe-back. Any screen using an X button instead
   of a back arrow should be reviewed — X blocks the native swipe gesture. Affected screens include
   recipe detail, cooking mode, login modal, and profile modals.
2. Header breadcrumbs: headers should show the current path so users know where they are
   (e.g. My Recipes > Tried > Test Folder). Decide on a consistent pattern across all screens.
3. Footer & bottom navigation: define the final navigation structure (tab bar vs. drawer vs. hybrid)
   before finalising header layout — the two decisions are coupled.
...
My Recipes — To Try / Tried folder system ✅
  Home screen: two large buttons (To Try green, Tried green) with counts.
  ...
  Folders persist independently of recipes via recipe_folders table — empty folders survive deletion,
  cross-list moves, and app restarts. Folders synced across MyRecipesScreen and RecipeDetailScreen.
  Move/rename/delete folders. Mark-as-tried from To Try view. Move back to To Try from Tried view.
  Recipe cards: title 1 line, prep time chip, Overall satisfaction stars (Tried only, inline).
  Action sheet: Rate / Change rating, Move to "To Try" group, Move to another folder, Delete.
  Tried action sheet shows "Tried on" date subtitle and correct Rate/Change rating label.
  Filter panel: search filters + Rating section (Rated/Not rated, min Overall score via stars).
  Sort by rating (low→high / high→low) available when Rated filter is active.
  DB: folder + search_filters on favourites and tried_recipes tables; recipe_folders table for persistence.
Shopping list ✅
...
My Recipes — tags ✅
  Tag picker on save (pageSheet). Suggested + custom tags.
