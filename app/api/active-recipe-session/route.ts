...
export async function POST(request: NextRequest) {
  try {
    const { userId, recipeId, recipeTitle, recipeData, substitutions } = await request.json()
    if (!userId || !recipeId || !recipeData) {
      return NextResponse.json({ error: "Missing required fields" }, { status: 400 })
    }
    await pool.query(
      `INSERT INTO active_recipe_session (user_id, recipe_id, recipe_title, recipe_data, substitutions)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         updated_at = NOW()`,
      [userId, recipeId, recipeTitle, JSON.stringify(recipeData), JSON.stringify(substitutions ?? [])]
    )
